    ...
    public String[] getPermissions() {
        if (permissions == null) {
            ...
            if (StringUtil.isNotBlank(baseDn) && StringUtil.isNotBlank(accountFilter)) {
                ...
                // The two spellings are not interchangeable: normalizing the whole name folds the
                // prefix as well, and only LdapManager knows whether this name was asserted by a
                // provider -- which decides whether a backslash in it is a NetBIOS qualifier to
                // drop or a character of the name. Assembling it here produced a second, differently
                // spelled permission for the same identity, in the same set as the first.
                //
